  ...
  */
  class Datatables_Lib
  {
    ...
    */
    ...
    {
      $iStart = $this->ci->input->get_post('start');
      $iLength = $this->ci->input->get_post('length');
    ...
    */
    private function get_ordering()
    {
      ...
      $Data = $this->ci->input->get_post('columns');


      if ($this->ci->input->get_post('order'))
        foreach ($this->ci->input->get_post('order') as $key)
          if($this->check_cType())
    ...
    */
    private function get_filtering()
    {
      ...
      $mColArray = $this->ci->input->get_post('columns');
      ...
      if($sSearch != ''){
        for($i = 0; $i < count($mColArray); $i++){
          if($mColArray[$i]['searchable'] == 'true' ){
            if($this->check_cType()){
              $sWhere .= "LOWER(".$this->select[$mColArray[$i]['data']] . ") LIKE BINARY LOWER('%" . $sSearch . "%') OR ";
            } else {
              $sWhere .= "LOWER(".$this->select[$this->columns[$i]] . ") LIKE BINARY LOWER('%" . $sSearch . "%') OR ";
            }
          }  
        }    
      }      
      ...
      // 個別搜尋
      $idv_column = array(); //儲存個別SQL條件
      for($i = 0; $i < count($mColArray); $i++){
        if($mColArray[$i]['search']['value'] !=''){
          $idv_column[] = "LOWER(".$this->select[$mColArray[$i]['data']] . ") LIKE BINARY LOWER('%" . $mColArray[$i]['search']['value'] . "%')";
        }
      }
      foreach($idv_column as $val){//最後做where
    ...
    */
    ...
    {
      $column = $this->ci->input->get_post('columns');
